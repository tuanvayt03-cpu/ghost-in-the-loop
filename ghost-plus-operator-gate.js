...
const q=(s,r=document)=>r.querySelector(s), qa=(s,r=document)=>[...r.querySelectorAll(s)], norm=v=>String(v||'').replace(/\s+/g,' ').trim(), now=()=>Date.now(), sleep=ms=>new Promise(r=>setTimeout(r,ms));
...
const S={key:'',gate:null,last:'',busy:false,msg:'',obs:null};
...
function style(){if(q('#ghostplus-gate-style'))return;const s=document.createElement('style');s.id='ghostplus-gate-style';s.textContent=`html[data-ghostplus-gate] #ghostplus-mini{box-shadow:0 0 0 3px rgba(239,68,68,.6),0 8px 24px rgba(15,23,42,.18)!important;background:#fff1f2!important;color:#991b1b!important}#ghostplus-gate{margin-top:6px;padding:7px;border:1px solid rgba(239,68,68,.4);border-radius:8px;background:#fff1f2;color:#7f1d1d;font:10px/1.35 system-ui}#ghostplus-gate .r{display:flex;gap:4px;margin-top:5px}#ghostplus-gate input{min-width:0;flex:1;font:10px system-ui;padding:4px}#ghostplus-gate button{font:10px system-ui;padding:4px 6px}`;document.documentElement.appendChild(s)}
function ui(){style();const h=q('#ghostplus-watch');if(!h)return null;let r=q('#ghostplus-gate',h);if(r)return r;r=document.createElement('div');r.id='ghostplus-gate';r.innerHTML=`<div><b data-t></b> <span data-id></span></div><div data-r style="margin-top:3px;max-height:54px;overflow:auto"></div><div class="r"><input data-note placeholder="Phản hồi/ghi chú"><button data-resume>Resume</button><button data-stop>Stop</button></div><div data-msg style="margin-top:3px"></div>`;h.appendChild(r);q('[data-resume]',r).onclick=e=>{if(!e.isTrusted)return;resume()};q('[data-stop]',r).onclick=e=>{if(!e.isTrusted)return;stop();clear(true)};return r}
...
async function resume(){if(S.busy)return;const g=S.gate||gate();if(!g)return;if(ctext()){S.msg='Ô nhập đang có draft. Hãy gửi/xóa draft trước khi Resume.';render();return}S.busy=true;const note=norm(q('#ghostplus-gate [data-note]')?.value||''),p=['[OPERATOR GATE RESOLVED]','A human explicitly authorized continuation.',note?`Operator response: ${note}`:'Continue from the exact paused checkpoint.','Do not restart or repeat completed work. Reconcile uncertain prior side effects before new ones.','Keep the Ghost protocol and end with exactly one valid terminal control line.'].join('\n');stop();await sleep(100);if(!setComposer(p)){S.busy=false;S.msg='Không stage được resume prompt; gate vẫn khóa.';render();return}if(g.type==='RECOVERY_EXHAUSTED'){try{window.__ghostPlusWatchdog?.resetRecoveryEpisode?.()}catch(_){}}if(g.h)saveAck(key(),g.h);save(key(),null);S.gate=null;document.documentElement.removeAttribute('data-ghostplus-gate');document.title=document.title.replace(/^[🔴🟠]\s+(HUMAN|RELAY|CONTEXT|AUTH|RECOVERY|BLOCKED)\s+·\s+/,'');const b=play(),before=users();try{b?.click()}catch(_){}const start=now();let ok=false;while(now()-start<4000){if(/^RUNNING\b/i.test(status())||users()>before){ok=true;break}await sleep(200)}S.busy=false;if(!ok){lock(g.type,{h:g.h,reason:g.reason,model:g.model});S.msg='Resume chưa xác nhận; gate đã khóa lại.';render()}else emit({id:`resumed:${g.id}`,type:'GATE_RESUMED',severity:'info',group:'operator',title:'Ghost+ resumed',text:g.id,episodeId:g.id,desktop:false})}
function evaluate(){const k=key();if(k!==S.key){S.key=k;S.gate=gate(k);S.last=''}const t=latest(),h=hash(t);if(t&&h!==S.last){S.last=h;const [ty,model]=term(t);if(ty==='human'&&ack(k)!==h)lock('HUMAN_REQUIRED',{h,reason:reason(t)});else if(ty==='relay'&&ack(k)!==h)lock('MODEL_RELAY',{h,reason:reason(t),model})}if(q('#ghostplus-context-boundary-state')&&!S.gate)lock('CONTEXT_BOUNDARY',{reason:'Context limit reached; manual handoff to a new chat is required.'});if(S.gate)enforce();else render()}
document.addEventListener('click',e=>{const b=e.target instanceof Element?e.target.closest('#gitl9 [data-a="play"]'):null,g=S.gate||gate();if(!b||!g)return;e.preventDefault();e.stopImmediatePropagation();S.msg=`Play bị khóa bởi ${g.type} · ${g.id}.`;render()},true);
try{S.obs=new MutationObserver(()=>queueMicrotask(evaluate));S.obs.observe(document.documentElement,{subtree:true,childList:true,characterData:true})}catch(_){}
setInterval(evaluate,700);
window.__ghostPlusSupervisor={version:'0.14',gate:()=>S.gate||gate(),isLocked:()=>!!(S.gate||gate()),canAutoAct:()=>!(S.gate||gate()),lock,emitAlert:emit,chatKey:key};
evaluate();
})();
